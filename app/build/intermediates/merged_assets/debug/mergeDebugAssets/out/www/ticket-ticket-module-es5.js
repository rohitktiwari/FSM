...
var TicketPageRoutingModule = /** @class */ (function () {
    function TicketPageRoutingModule() {
    }
...
var TicketPage = /** @class */ (function () {
    function TicketPage(network, nativeStorage, modalController, router, http, navCtrl, RestService, callNumber, sms, androidPermissions) {
        var _this = this;
    ...
    TicketPage.prototype.showMap = function (lat, lng) {
    ...
    TicketPage.prototype.getOfflineTickets = function () {
        var _this = this;
    ...
    TicketPage.prototype.getTickets = function () {
        var _this = this;
        ...
        this.RestService.requestViaGet('app/EngineerTickets/GetEngineerFilterTickets/' + id + '/' + this.ticketType, '').then(function (response) {
            _this.ticketsArr = [];
            _this.events = response;
            localStorage.setItem("termsData", _this.events.TermsAndConditions);
            _this.ticketsArr = _this.events.result;
            _this.preTicketArr = _this.ticketsArr;
            console.log(_this.ticketsArr);
        });
    };
    TicketPage.prototype.showTicket = function () {
    ...
    TicketPage.prototype.getFilterTickets = function (val) {
        var newArr = [];
        var selectedDate = new Date(val).toLocaleString().split(',')[0]; //toDateString()
        for (var i = 0; i < this.preTicketArr.length; i++) {
    ...
    TicketPage.prototype.dialNumber = function (num) {
        this.callNumber.callNumber(num, true)
            .then(function (res) { return console.log('Launched dialer!', res); })
            .catch(function (err) { return console.log('Error launching dialer', err); });
    };
    TicketPage.prototype.sendMsg = function (num) {
        var option = {
        ...
        if (this.sms.hasPermission) {
            this.sms.send(num, 'Hello world!', option).then(function (res) {
                console.log(res);
                // alert('Messange Sent!')
            }).catch(function (err) {
    ...
    TicketPage.prototype.ticketDetail = function (val) {
        localStorage.setItem('ticket', JSON.stringify(val));
        this.navCtrl.navigateForward("/ticket-detail");
    };
    TicketPage.prototype.openCalModal = function () {
                        ...
                        modal.onWillDismiss().then(function (dataReturned) {
                            ...
                            if (dataReturned.data) {
                                if (dataReturned.data.date) {
                                    _this.getFilterTickets(dataReturned.data.date);
                                }
                            }
                        });
