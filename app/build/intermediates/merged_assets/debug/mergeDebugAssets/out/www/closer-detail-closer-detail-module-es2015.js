...
let CloserDetailPageRoutingModule = class CloserDetailPageRoutingModule {
};
...
let CloserDetailPageModule = class CloserDetailPageModule {
};
...
let CloserDetailPage = class CloserDetailPage {
    constructor(toastController, transfer, mediaCapture, sanitizer, platform, filePicker, nativeStorage, network, jsonProvider, alertController, filePath, fileChooser, loadingController, file, base64, location, camera, actionSheetController, RestService) {
    ...
    ngOnInit() {
        ...
        this.nativeStorage.getItem('closerArr').then(data => {
            console.log(data);
            this.offlineCloserArr = data;
        }, error => console.error(error));
        if (localStorage.modeType == 'Offline' && this.network.type == 'none') {
            this.nativeStorage.getItem('offlineAllData').then(data => {
                console.log(data);
                this.paymentModeArr = data.Item5;
                this.taxArr = data.Item6;
            }, error => console.error(error));
        }
        else {
            this.getCloserId();
    ...
    getGSTList() {
        ...
        this.RestService.requestViaGet('app/EngineerTickets/GetTax/' + Token, '').then((response) => {
            console.log(response);
            this.apiResponse = response;
            if (this.apiResponse.Message == "success") {
                this.taxArr = this.apiResponse.result;
            }
            else {
            }
    ...
    getPayTypeList() {
        var Token = JSON.parse(localStorage.userData).Token;
        this.RestService.requestViaGet('app/EngineerTickets/GetPaymentModet/' + Token, '').then((response) => {
            console.log(response);
            this.apiResponse = response;
            if (this.apiResponse.Message == "success") {
                this.paymentModeArr = this.apiResponse.result;
            }
            else {
            }
            //  alert(response.MessageDescription);
    ...
    getCloserId() {
        {
            ...
            this.RestService.postServices('/app/EngineerTickets/PostCloserDetails/' + Token, data).then((response) => {
                ...
                this.apiResponse = response;
                if (this.apiResponse.Message == "success") {
                    this.closerId = this.apiResponse.result.CloserID;
                    this.ticketFlag = this.apiResponse.result.TicketFlag;
                    this.CloserPrefix = this.apiResponse.result.CloserPrefix;
                }
                else {
                }
            });
    ...
    uploadPic(i) {
        ...
        this.camera.getPicture(options).then((imageData) => {
            ...
            this.attachedItem[i] = imageData;
            this.capturedPic[i] = imageData;
    ...
    PostCloserAttachment() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            if (this.attachedItem.length == 0) {
                const alert = yield this.alertController.create({
                    header: '',
    ...
    submitCloserDoc(data) {
        var Token = JSON.parse(localStorage.userData).Token;
        this.RestService.postServices('app/EngineerTickets/PostCloserAttachment/' + Token, data).then((response) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            console.log(response);
            this.apiResponse = response;
            if (this.apiResponse.Message == "success") {
    ...
    submitCloser() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            var gstAmount = this.amount * this.gst / 100;
            var ticketData = JSON.parse(localStorage.ticket);
            var userData = JSON.parse(localStorage.userData);
            if (this.otherName != '' || this.serviceaBy.includes('Other')) {
                var str = this.serviceaBy.toString() + ',' + this.otherName;
            }
            else {
                str = this.serviceaBy.toString();
            }
            var serviceaBy = str.replace(",Other", "");
            if (localStorage.modeType == 'Offline' && this.network.type == 'none') {
                var offlineData = {
                    "CloserID": '',
                    "TicketId": ticketData.Id,
                ...
                console.log(data);
                this.RestService.putServices('app/EngineerTickets/PutCloserDetails/' + userData.Token, data).then((response) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                    console.log(response);
                    this.apiResponse = response;
                    if (this.apiResponse.Message == "success") {
                        const alert = yield this.alertController.create({
    ...
    recordAudio(i) {
        var thisJS = this;
        this.mediaCapture.captureAudio().then((data) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            if (data.length > 0) {
                console.log('audio', data);
                let audioFile = data[0];
                let fileName = data[0].fullPath.substring(data[0].fullPath.lastIndexOf('/') + 1);
                let dir = audioFile['localURL'].split('/');
                let fileNameEXT = audioFile['localURL'].substring(audioFile['localURL'].lastIndexOf('.') + 1);
                this.ext[i] = fileNameEXT;
                if (this.platform.is('ios')) {
                    thisJS.base64.encodeFile(audioFile['fullPath']).then(base64String => {
                        console.log(base64String);
                        this.isAudio[i] = true;
                        this.attachedItem[i] = base64String.split(',')[1];
                    }).catch(err => {
                        console.log(err);
                    });
                }
                    ...
                    this.filePath.resolveNativePath(audioFile['fullPath'])
                        .then(filePath => {
                        console.log(filePath);
                        thisJS.base64.encodeFile(filePath).then(base64String => {
                            console.log(base64String);
                            this.isAudio[i] = true;
                            this.attachedItem[i] = base64String.split(',')[1];
                        }).catch(err => {
...
CloserDetailPage.ctorParameters = () => [
...
let JsonStoreService = class JsonStoreService {
    constructor(nativeStorage, http, alertCtrl, loadingCtrl) {
        this.nativeStorage = nativeStorage;
        this.http = http;
    ...
    jsonStoreData(setKey, JSONKey, jsonData, collectionName) {
        // var addOptions = {};
        // jsonData = {
    ...
    jsonStoregetDataByKey(SearchBy, collName) {
        try {
            var collections = {
                AllMasterSyncData: {
                    searchFields: { collectionKey: 'string' }
...
JsonStoreService.ctorParameters = () => [
...
//# sourceMappingURL=closer-detail-closer-detail-module-es2015.js.map