...
let ViewCloserPageRoutingModule = class ViewCloserPageRoutingModule {
};
...
let ViewCloserPageModule = class ViewCloserPageModule {
};
...
let ViewCloserPage = class ViewCloserPage {
    constructor(navCtrl, loadingCtrl, transfer, file, fileOpener, zone, alertCtrl) {
    ...
    downloadFile(url) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (thisJS.platform.is('ios')) {
        
              //check file exists on device or not
        
              this.file.checkFile(this.file.documentsDirectory, name).then((files) => {
        
                //file open in default pdf viewer
        
                this.fileOpener.open(this.file.documentsDirectory + name, 'application/pdf')
                ...
                //showing downlaod % in loader
        
                fileTransfer.onProgress((progressEvent) => {
                  var percent = progressEvent.loaded / progressEvent.total * 100;
                  thisJS.zone.run(() => {
                    this.percentage = Math.round(percent);
                    loader.setContent("Downloading " + this.percentage + "%");
                  });
            ...
            //check file exists on device or not
            // this.file.checkFile(this.file.externalDataDirectory, name).then((files) => {
            //file open in default pdf viewer
            //   this.fileOpener.open(this.file.externalDataDirectory + name, 'application/pdf')
            //     .then(() => console.log('File is opened'))
            //     .catch(e => console.log('Error opening file', e));
            // }).catch((err) => {
...
//# sourceMappingURL=view-closer-view-closer-module-es2015.js.map