...
let CloserDetailPage = class CloserDetailPage {
    constructor(domSanitizer, toastController, transfer, mediaCapture, sanitizer, platform, filePicker, nativeStorage, network, jsonProvider, alertController, filePath, fileChooser, loadingController, file, base64, location, camera, actionSheetController, RestService, zone) {
    ...
    stopAudio() {
        ...
        if (this.platform.is('android')) {
            ...
            if (response.base64) {
                that.zone.run(() => {
                    this.audioUrl[i] = response.path;
                    this.ext[i] = 'mp3';
                    this.isAudio[i] = true;
                    this.attachedItem[i] = response.base64;
                });
            }
        }
        else if (this.platform.is('ios')) {
    ...
    getProgress(i) {
        ...
        setTimeout(() => {
            if (this.play[i]) {
                if (this.progress[i] >= this.audioDuration[i]) {
                    this.pauseAudio(i);
                }
                else {
                    this.getProgress(i);
